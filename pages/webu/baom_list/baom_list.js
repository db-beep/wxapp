...
Page({
    ...
    async onLoad() {
        this.setData({
            ...
            ,stateOptionList:[
                {id:'',label:'请选择'} ,{id:'待审核', label:'待审核'} ,{id:'通过', label:'通过'} ,{id:'拒绝', label:'拒绝'}            ]
        })
    ...
    async loadData() {
        ...
        try {
            ...
            const res = await Http.get('/webu/baom/list', quertData);
            if (res.success) {
    ...
    async handleDel(e) {
        ...
        wx.showModal({
            ...
            success: async (res) => {  // 使用箭头函数
                if (res.confirm) {
                    let {success} = await Http.post('/webu/baom/delete?id=' + id)
                    if(success){
