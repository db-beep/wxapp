...
Page({
    ...
    async loadData(id) {
        let {
            data
        } = await Http.get("/webu/news/detail?id=" + id);
        this.setData({
            detail: data.entity,
    ...
    async toggleStar() {
        ...
        let {success} = await Http.post('/web/star', {action: isStar?'cancelStar':'star', id:this.data.detail.id, entityName:'news', op:'收藏'})
        if(success){
            this.setData({
                startBtnShow: isStar,
    ...
    async togglePraise() {
        ...
        let {success} = await Http.post('/web/star', {action: isPraise?'cancelStar':'star', id:this.data.detail.id, entityName:'news', op:'点赞'})
        if(success){
            this.setData({
    ...
    onShareAppMessage() {
        return {
            ...
            path: `/pages/news_detail_web/news_detail_web?id=${this.data.detail.id}` //分享路径
        };
    },
    //bool变化  新增修改等页面的显示
    boolChange(e) {
